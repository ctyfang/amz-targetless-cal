...
class Calibrator:
    ...
    def __init__(self, calib_dir, input_dir, frame):
        self.pc = np.fromfile(str(input_dir) + '/velodyne_points/data/' + str(frame).zfill(10) + '.bin',
        ...
        self.img = cv.imread(str(input_dir) + '/image_00/data/' + str(frame).zfill(10) + '.png')
    ...
    def pc_detect(self, thresh=0.6, num_nn=100, rad_nn=0.1, visualize=False):
        """Compute edge scores for pointcloud, get edge points via thresholding"""
        ...
        boundary_mask = [(edge_idx not in pc_boundary_idxs) for edge_idx in self.pc_edge_idxs]
        self.pc_edge_idxs = self.pc_edge_idxs[boundary_mask]

        pc_nonbound_edge_scores = np.delete(self.pc_edge_scores, pc_boundary_idxs, axis=0)
    ...
    def pc_to_pixels(self):
        ...
        # TODO: Perform transform without homogeneous term if too memory intensive
