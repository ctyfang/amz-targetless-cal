...
if __name__ == '__main__':
    ...
    pcfiles = sorted(
        glob(os.path.join(cfg.dir_data, cfg.lidar + '_lidar_filtered',
                          '*.bin')))
    imgfile = sorted(
        glob(
            os.path.join(cfg.dir_data, cfg.camera + '_camera_filtered',
                         '*.png')))
    ...
    os.makedirs(extrinsics, exist_ok=True)
    with open(
            os.path.join(pwd, 'extrinsics',
                         f'extrinsics_{cfg.lidar}_{cfg.camera}.npz'),
            'wb') as f:
        np.savez(f, R=R, t=t)
    ...
    if cfg.check_projection:
        with np.load(
                os.path.join(
                    pwd, 'extrinsics',
                    f'extrinsics_{cfg.lidar}_{cfg.camera}.npz')) as data:
            R = data['R'].reshape((3, 1))
