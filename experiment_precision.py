...
def expe_plot(tau_data, tau_init, tau_gt, title, filename, alpha=0.5):
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == '--plot':
        ...
        expe_plot(tau_data, tau_inits, tau_gt, 'GMM only',
                  os.path.join(LOG_DIR, 'exp_gmm_only'))
        sys.exit()

        with open('generated/logs/set_3_t_10_r_05/tau_initset_3_t_10_r_05.npy',
                  'rb') as f:
            tau_inits = np.load(f)[:200, :]
        with open('generated/logs/set_3_t_10_r_05/tau_dataset_3_t_10_r_05.npy',
                  'rb') as f:
            tau_data = np.load(f)
        ...
        table = PrettyTable()
        table.field_names = [
            "X_med", "X_std", "Y_med", "Y_std", "Z_med", "Z_std", "angle_med",
            "angle_std", "axis_med", "axis_std"
        ]

        x_error = tau_data[:, 3] - tau_gt[3]
        y_error = tau_data[:, 4] - tau_gt[4]
        z_error = tau_data[:, 5] - tau_gt[5]
        angle_error = np.linalg.norm(tau_data[:, :3], ord=2,
                                     axis=1) - np.linalg.norm(tau_gt[:3], ord=2)
        ...
        with open(os.path.join(LOG_DIR, 'error_metrics.txt'),
                  'w') as table_file:
